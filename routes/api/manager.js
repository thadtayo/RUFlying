...
router.get('/active_flights', (req, res) => {
    ...
    if(true) {
        ...
        con.query(`SELECT flight_num, COUNT(flight_num) AS num_reservations, airline_id, num_seats, working_days, fares, on_schedule, occupancy FROM Reservations INNER JOIN Flights USING (flight_num) GROUP BY flight_num ORDER BY num_reservations DESC LIMIT 5;`, function(err, results, fields) {
            if (err) throw err;
            res.json(results);
        });
    } else {
...
router.get('/on_time', (req, res) => {
    ...
    if(true) {
        ...
        con.query(`SELECT * FROM Flights WHERE on_schedule='true';`, function (err, results, fields) {
            if(err) throw err;
            res.json(results);
        });
